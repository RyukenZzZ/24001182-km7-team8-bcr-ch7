...
function Index() {
    ...
    const { data: carsData, isSuccess: carsSuccess, isPending: carsPending } = useQuery({
        queryKey: ["cars"],
        queryFn: () => getCars(),
        enabled: !!token,
    });
    
    const { data: modelsData, isSuccess: modelsSuccess, isPending: modelsPending } = useQuery({
        queryKey: ["models"],
        queryFn: () => getModels(),
        enabled: !!token,
    });
    
    const { data: manufacturesData, isSuccess: manufacturesSuccess, isPending: manufacturesPending } = useQuery({
        queryKey: ["manufactures"],
        queryFn: () => getManufactures(),
        enabled: !!token,
    });
    
    const { data: typesData, isSuccess: typesSuccess, isPending: typesPending } = useQuery({
        queryKey: ["types"],
        queryFn: () => getTypes(),
        enabled: !!token,
    });
    ...
    }, [carsData, modelsData, manufacturesData, typesData, carsSuccess, modelsSuccess, manufacturesSuccess, typesSuccess]);

    const isLoading = carsPending || modelsPending || manufacturesPending || typesPending;
    ...
    return (
        <Row className="mt-4">
            <Col>
                ...
                <div className="mb-4">
                    <button className="btn btn-outline-success ms-2" onClick={() => setSelectedData("cars")}>Show Cars</button>
                    <button className="btn btn-outline-secondary ms-2" onClick={() => setSelectedData("models")}>Show Models</button>
                    <button className="btn btn-outline-secondary ms-2" onClick={() => setSelectedData("manufactures")}>Show Manufactures</button>
                    <button className="btn btn-outline-secondary ms-2" onClick={() => setSelectedData("types")}>Show Types</button>
                </div>
